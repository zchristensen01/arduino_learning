...
const uint8_t PIN_WLK = 5;
const uint8_t PIN_BTN = 2;

const unsigned long WALK_MS = 4000;
const uint8_t WALK_FLASHES = 4;

unsigned int pressCount = 0;

enum class Light : uint8_t { RED, GREEN, YELLOW, WALK }; // create the Light type
...
void setLight(bool r , bool y, bool g, bool w) { // bool -> digitalWrite conversion works because HIGH and LOW is just #defines for 1 and 0
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    case 'r':
      state = Light::RED;
      stateStart = millis();
      Serial.println("! forced RED");
      break;
    case 'g':
      state = Light::GREEN;
      stateStart = millis();
      Serial.println("! forced GREEN");
      break;
    case 'y':
      state = Light::YELLOW;
      stateStart = millis();
      Serial.println("! forced YELLOW");
      break;
    case 'w':
      state = Light::WALK;
      stateStart = millis();
      Serial.println("! forced WALK");
      break;
    }
  }
  ...
  */
  switch (state) {
    case Light::RED:
      setLight(true, false, false, false);
      if (elapsed >= 4000) {
        state = Light::GREEN; 
        stateStart = millis();
    ...
    case Light::GREEN:
      setLight(false, false, true, false);
    ...
    case Light::YELLOW:
      ...
      break;
    case Light::WALK: {
      const unsigned long HALF = WALK_MS / (2 * WALK_FLASHES); // 500
      bool blinkOn = ((elapsed / HALF) % 2 == 0);
      setLight(true, false, false, blinkOn);
      if (elapsed >= WALK_MS) {
        state = Light::GREEN; 
        stateStart = millis();
